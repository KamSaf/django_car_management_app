...
def get_fuel_prices() -> list:
    ...
    return [float(item.text.strip()[0:4].replace(',', '.')) for item in soup.find_all(class_='price')]
...
def save_prices(prices: list) -> None:
    ...
    FuelPrices.objects.create(pb95=prices[0], pb98=prices[1], diesel=prices[2], diesel_premium=prices[3], lpg=prices[4], date=timezone.now()).save()
